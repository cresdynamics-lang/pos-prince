...
if [[ -f "${APP_DIR}/backend/.env" ]]; then
  ...
  if [[ -n "${DB_URL}" ]]; then
    ...
    for f in 010_activity_log.sql 011_catalog_prices.sql 012_remove_demo_sales.sql 013_client_catalog_prices.sql 014_staff_role_permissions.sql 015_belts_caps_no_sizes.sql; do
